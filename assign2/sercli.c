/*
   Author: Hongru Li 215245830 version1
...
struct	quene{
	...
	int		number_of_wait_client;	//a signal like signal(S) and wait(S);
...
void* clocker(void* arg){
	...
	for(i=0; i < ROUNDS; i++){
		while(1){
			...
			//if there is no one waiting, wake up to emit next tick.
		if(pthread_mutex_lock(&quene->mutex) != 0){
			pthread_exit((void*) 1);
		}
		//the clock may already arrive here when the last server is still running
		//the mutex make the clock wait for the next tick until the last one 
		//sleep
...
void* client_thread(void* arg){
	...
	for(i = 0; i< ROUNDS; i++){
		...
		//all clients thread wake up together but only one can write the quene
		if(pthread_mutex_lock(&quene->mutex) != 0){
...
void* server_thread(void* arg){
	...
	for(i = 0; i< ROUNDS; i++){
		...
		//all servers wake up after the last client finish its job
		//only one server can consume the tasks.
			pthread_exit((void*) 0);
		}
